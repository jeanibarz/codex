...
fn main() {
    // macOS-specific link flags inherited from upstream's build.rs.
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("macos") {
        println!("cargo:rustc-link-arg=-ObjC");
    }

    emit_codex_build_version();
}

fn emit_codex_build_version() {
...
fn register_rerun_inputs(repo_root: &Path) {
