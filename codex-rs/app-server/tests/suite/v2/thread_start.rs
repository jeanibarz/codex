...
#[test]
fn thread_start_inherits_settings_file_hooks_from_embedded_process() -> Result<()> {
    const TEST_STACK_SIZE_BYTES: usize = 8 * 1024 * 1024;

    let handle = std::thread::Builder::new()
        .name("thread_start_inherits_settings_file_hooks_from_embedded_process".to_string())
        ...
        .spawn(|| -> Result<()> {
            let runtime = tokio::runtime::Builder::new_multi_thread()
                ...
                .enable_all()
                .build()?;
            runtime.block_on(thread_start_inherits_settings_file_hooks_from_embedded_process_impl())
        })?;

    match handle.join() {
        Ok(result) => result,
        Err(_) => Err(anyhow::anyhow!(
            "thread_start_inherits_settings_file_hooks_from_embedded_process thread panicked"
        )),
    }
}

async fn thread_start_inherits_settings_file_hooks_from_embedded_process_impl() -> Result<()> {
