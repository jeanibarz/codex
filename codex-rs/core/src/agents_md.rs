...
/// Resolves AGENTS.md and related instruction source files.
    ...
    for turn_environment in &environments.turn_environments {
        ...
        .await
        {
            ...
            Err(e) => {
                error!(
                    environment_id = turn_environment.environment_id,
                    "error trying to discover conditional rules: {e:#}"
                );
            }
        }
    }
...
impl<'a> AgentsMdManager<'a> {
    ...
    pub async fn instruction_sources(&self, fs: &dyn ExecutorFileSystem) -> Vec<AbsolutePathBuf> {
        ...
        match rule_paths(self.config, fs, &self.config.cwd).await {
...
async fn read_conditional_rules(
    ...
    // Conditional rules share the project-doc budget with AGENTS.md for each
    // environment so a workspace cannot exceed `project_doc_max_bytes` by
